...
def train(rank, args, chkpt_path, hp, hp_str):
    ...
    optim_g = tf.keras.optimizers.AdamW(model_g.parameters(),
                                lr=hp.train.learning_rate, betas=hp.train.betas, eps=hp.train.eps)
    optim_d = tf.keras.optimizers.AdamW(model_d.parameters(),
                                lr=hp.train.learning_rate, betas=hp.train.betas, eps=hp.train.eps)
    ...
    scheduler_g = tf.keras.optimizers.schedules.ExponentialDecay(optim_g, gamma=hp.train.lr_decay, last_epoch=init_epoch-2)
    scheduler_d = tf.keras.optimizers.schedules.ExponentialDecay(optim_d, gamma=hp.train.lr_decay, last_epoch=init_epoch-2)
