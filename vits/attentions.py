...
#         self.conv_q = tf.keras.layers.Conv1D(channels, channels, 1)
#         self.conv_k = tf.keras.layers.Conv1D(channels, channels, 1)
#         self.conv_v = tf.keras.layers.Conv1D(channels, channels, 1)
#         self.conv_o = tf.keras.layers.Conv1D(channels, out_channels, 1)
#         self.drop = tf.keras.layers.Dropout(p_dropout)
...
#                 tf.random.normal(n_heads_rel, window_size * 2 + 1, self.k_channels)
#                 * rel_stddev
#             )
...
#         self.conv_q.kernel_initializer=tf.keras.initializers.GlorotUniform()
#         self.conv_v.kernel_initializer=tf.keras.initializers.GlorotUniform()
#         self.conv_k.kernel_initializer=tf.keras.initializers.GlorotUniform()
#         if proximal_init:
#             with tf.no_gradient():
#                 self.conv_k.kernel=self.conv_q.kernel
...
#         scores = tf.matmul(query / math.sqrt(self.k_channels), key.transpose(-2, -1))
...
#         p_attn = tf.keras.activations.softmax(scores, dim=-1)  # [b, n_h, t_t, t_s]
...
#         ret: [b, h, l, m]
#         """
#         ret = tf.matmul(x, y.unsqueeze(0).transpose(-2, -1))
#         return ret
...
#         x = tf.pad(x, commons.convert_pad_shape([[0, 0], [0, 0], [0, 0], [0, 1]]))
...
#         x_flat = tf.pad(x_flat, commons.convert_pad_shape([[0, 0], [0, 0], [length, 0]]))
