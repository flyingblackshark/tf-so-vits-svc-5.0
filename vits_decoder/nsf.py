...
#             sine_1 = tf.roll(pure_sine, shifts=1, axis=1)
#             uv_1 = tf.roll(uv, shifts=1, axis=1)
#             uv_1[:, 0, :] = 0
#             sine_2 = tf.roll(pure_sine, shifts=-1, axis=1)
#             uv_2 = tf.roll(uv, shifts=-1, axis=1)
#             uv_2[:, -1, :] = 0
...
#             pulse_noise = tf.random.normal(shape=pure_sine.shape,dtype=pure_sine.dtype) * self.noise_std
...
#     Note: this is based on torch.nn.functional.tf.keras.layers.Conv1D
...
#         signal_pad = tf.pad(signal.permute(0, 2, 1), (padding_length, 0))
#         # prepare system impulse response as (dim, 1, length2)
#         # also flip the impulse response
#         ir = tf.reverse(system_ir.unsqueeze(1).permute(2, 1, 0), dims=[2])
#         # convolute
#         output = tf.keras.layers.tf.keras.layers.Conv1D(signal_pad, ir, groups=groups)
#         return output.permute(0, 2, 1)


# class CyclicNoiseGen_v1(tf.Module):
#     """CyclicnoiseGen_v1
...
#         if (uv < 1).all():
#             # all unvoiced
#             cyc_noise = tf.zeros_like(sine_wav)
#         else:
...
class SineGen(tf.keras.layers.Layer):
    ...
    def _f02sine(self, f0_values):
        ...
        if not self.flag_for_pulse:
            ...
            sines = tf.sin(
                tf.cumsum(rad_values + cumsum_shift, axis=1) * 2 * np.pi
            )
        else:
            ...
            uv_1 = tf.roll(uv, shifts=-1, dims=1)
            uv_1[:, -1, :] = 1
            u_loc = (uv < 1) * (uv_1 > 0)

            # get the instantanouse phase
            tmp_cumsum = tf.cumsum(rad_values, dim=1)
            ...
            i_phase = tf.cumsum(rad_values - tmp_cumsum, dim=1)
    ...
    def call(self, f0):
       ...
       # with tf.no_gradient("Size"):
        f0_buf = tf.zeros([f0.shape[0], f0.shape[1], self.dim]).numpy()
        # fundamental component
        f0_buf[:, :, 0] = f0[:, :, 0]
        for idx in np.arange(self.harmonic_num):
             f0_buf[:, :, idx + 1]=f0_buf[:, :, 0] * (idx + 2)
            # idx + 2: the (idx+1)-th overtone, (idx+2)-th harmonic
            # for i in range(f0_buf.shape[0]):
            #     for j in range(f0_buf.shape[1]):
           # tf.py_function(f0_buf, inp=[f0_buf, [:, :, idx + 1], f0_buf[:, :, 0] * (idx + 2)], Tout=f0_buf.dtype)
           # f0_buf[:, :, idx + 1] = f0_buf[:, :, 0] * (idx + 2)
        ...
        noise = noise_amp * tf.random.normal(sine_waves.shape)
